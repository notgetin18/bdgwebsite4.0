...
const AddNewBank = ({ toggleBankVerificationHandler }: any) => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        accountName: Yup.string().required('Account Holders Name is required').min(3, 'Name must be at least 3 characters').max(50, 'Name cannot exceed 50 characters').matches(/^[a-zA-Z0-9\s]+$/, 'Invalid Account Holder Name'),
                ...
                {({ values, errors, touched, setFieldValue, handleChange, handleBlur, handleSubmit }) => (
                    <form onSubmit={(e) => { e.preventDefault(); }}>
                        <div className=''>
                            ...
                            <select className='text-black' name="bankName" value='' onChange={(data) => {
                                ...
                                {bankList.map((item, key) => {
                                    return (<>
                                        <option className='text-black' key={item._id} value={item.name}>{item.name}</option>
                                    </>)
                                })};
                            </select>
                            {!otherBankName && <ErrorMessage name="bankName" className='text-red-500' component="div" />}
                        </div>
                        {otherBankName && (
                            <div className=''>
                                <label >Bank Name</label><br />
                                <input
                                    ...
                                    onBlur={handleBlur}
                                />
                                <ErrorMessage name="bankName" className='text-red-500' component="div" />
                            </div>
                        )}
                        <div className=''>
                            ...
                            <input
                               className='text-black'
                                ...
                                onBlur={handleBlur}
                            />
                            <ErrorMessage name="accountName" className='text-red-500' component="div" />
                        </div>
                        <div className=''>
                            ...
                            <input
                                ...
                                onBlur={handleBlur}
                            />
                            <ErrorMessage name="accountNumber" className='text-red-500' component="div" />
                        </div>
                        <div className=''>
                            <label >IFSC</label><br />
                            <input
                            ...
                            />
                            <ErrorMessage name="ifsc" className='text-red-500' component="div" />
                        </div>

                        <div className='mt-2'>
                            <button type="submit" onClick={() => { handleSubmit() }} disabled={isSubmitting}>
                                <div className='px-3 py-1 rounded border-2 border-yellow-400 text-yellow-400'>Verify</div>
                            </button>
                        </div>
                    </form>
                )}
